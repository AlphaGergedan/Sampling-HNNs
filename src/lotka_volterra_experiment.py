"""
This file conducts all the lotka volterra experiments
"""
...
####################################################################################################
#       EXPERIMENT ZERO-CENTERED LOTKA-VOLTERRA ON DOMAIN [-2,2]x[-2,2] NETWORK-WIDTH SCALING
...
for network_width in range(100, 2100, 100): #[100, 200, .. , 2000]
    START_MODEL_RANDOM_SEED = kwargs["start_model_random_seed"]
    START_DATA_RANDOM_SEED = kwargs["start_data_random_seed"]

    domain_params = SampledDomainParams(lotka_volterra, q_lims, p_lims, 10000, 10000, kwargs["repeat"], START_DATA_RANDOM_SEED)
    ...
    dump(models, os.path.join(kwargs["save_dir"], f"small_domain_zero_centered_models_hidden_{network_width}.pkl"))
    dump(experiment, os.path.join(kwargs["save_dir"], f"small_domain_zero_centered_experiment_hidden_{network_width}.pkl"))
    print(f"-> finished network width {network_width}")

####################################################################################################
#       EXPERIMENT ZERO-CENTERED LOTKA-VOLTERRA ON DOMAIN [-5,5]x[-5,5] NETWORK-WIDTH SCALING
...
for network_width in range(100, 2100, 100): #[100, 200, .. , 2000]
    ...
    dump(models, os.path.join(kwargs["save_dir"], f"large_domain_zero_centered_models_hidden_{network_width}.pkl"))
    dump(experiment, os.path.join(kwargs["save_dir"], f"large_domain_zero_centered_experiment_hidden_{network_width}.pkl"))
    print(f"-> finished network width {network_width}")

####################################################################################################
#       EXPERIMENT FIVE-CENTERED LOTKA-VOLTERRA ON DOMAIN [0,8]x[0,8] NETWORK-WIDTH SCALING
...
for network_width in range(100, 2100, 100): #[100, 200, .. , 2000]
    ...
    dump(models, os.path.join(kwargs["save_dir"], f"five_centered_models_hidden_{network_width}.pkl"))
    dump(experiment, os.path.join(kwargs["save_dir"], f"five_centered_experiment_hidden_{network_width}.pkl"))
    print(f"-> finished network width {network_width}")
