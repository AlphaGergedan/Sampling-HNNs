"""
This file conducts all the henon heiles experiments
"""
...
####################################################################################################
#       EXPERIMENT HENON HEILES ON DOMAIN [-5,5]x[-5,5] ALPHA SCALING
...
for n in range(0, 11):
    alpha = n/10   # [0, 0.1, ..., 0.9, 1.0]
    henon_heiles.alpha = alpha

    START_MODEL_RANDOM_SEED = kwargs["start_model_random_seed"]
    START_DATA_RANDOM_SEED = kwargs["start_data_random_seed"]

    domain_params = SampledDomainParams(henon_heiles, q_lims, p_lims, 20000, 20000, kwargs["repeat"], START_DATA_RANDOM_SEED)
    model_params = SampledModelParams(ActivationType.TANH, network_width=5000, resample_duplicates=kwargs["resample_duplicates"], rcond=kwargs["rcond"], elm_bias_start=kwargs["min_input"], elm_bias_end=kwargs["max_input"], start_model_random_seed=START_MODEL_RANDOM_SEED)
    ...
    dump(models, os.path.join(kwargs["save_dir"], f"models_alpha_{alpha}.pkl"))
    dump(experiment, os.path.join(kwargs["save_dir"], f"experiment_alpha_{alpha}.pkl"))
    print(f"-> finished network width {alpha}")
