"""
This file conducts all the single pendulum experiments
"""
...
####################################################################################################
#           EXPERIMENT SINGLE PENDULUM ON DOMAIN [-2pi,2pi]x[-1,1] NETWORK-WIDTH SCALING


single_pendulum = SinglePendulum()
q_lims = [ [-2*np.pi, 2*np.pi] ]
p_lims = [ [-1., 1.] ]
...
for network_width in range(100, 2100, 100): #[100, 200, .. , 2000]
    ...
    domain_params = SampledDomainParams(single_pendulum, q_lims, p_lims, 10000, 10000, kwargs["repeat"], START_DATA_RANDOM_SEED)
    ...
    dump(models, os.path.join(kwargs["save_dir"], f"small_domain_models_hidden_{network_width}.pkl"))
    dump(experiment, os.path.join(kwargs["save_dir"], f"small_domain_experiment_hidden_{network_width}.pkl"))
    print(f"-> finished network width {network_width}")


####################################################################################################
#           EXPERIMENT SINGLE PENDULUM ON DOMAIN [-2pi,2pi]x[-6,6] NETWORK-WIDTH SCALING
...
for network_width in range(100, 2100, 100): #[100, 200, .. , 2000]
    ...
    dump(models, os.path.join(kwargs["save_dir"], f"large_domain_models_hidden_{network_width}.pkl"))
    dump(experiment, os.path.join(kwargs["save_dir"], f"large_domain_experiment_hidden_{network_width}.pkl"))
    print(f"-> finished network width {network_width}")
